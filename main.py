...
def get_bounds(image: Image.Image) -> tuple[int, int, int, int]:
    ...
    for i in range(0, len(bin_rows), 4):
        ...
        # print(''.join([str(val) for val in compressed[i // 4]]))
    left = sorted(set([compressed[i].index('0', compressed[i].index('0')) for i in range(len(compressed)) if '0' in compressed[i]]))[1] * 4
    right = sorted(set([-compressed[i][::-1].index('0', compressed[i][::-1].index('0')) % len(compressed[i]) for i in range(len(compressed)) if '0' in compressed[i]]))[-2] * 4
    lower = sorted(set([i for i in range(len(compressed)) if '0' in compressed[i]]))[1] * 4
    upper = sorted(set([i for i in range(len(compressed)) if '0' in compressed[i]]))[-2] * 4
...
def normalize_image(image: Image.Image, threshhold_modifier: int = 0) -> Image.Image:
    ...
    if threshhold_modifier > 0:
        ...
        res = average_images([a, b])
    else:
        res = mask_image_thresh(image, thresh_base % 256)

    res = denoise(res)
    res_shape = get_bounds(res)
    res = res.crop(res_shape)
    res = res.resize((512, 512))
    return res
